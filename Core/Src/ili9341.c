...
void ili9341_init(void)
{
	...
	//Reset the display
	HAL_GPIO_WritePin(DISPL_RESET_GPIO_Port, DISPL_RESET_Pin, GPIO_PIN_RESET);
	osDelay(100);
	HAL_GPIO_WritePin(DISPL_RESET_GPIO_Port, DISPL_RESET_Pin, GPIO_PIN_SET);
	osDelay(100);
...
void ili9341_enable_backlight(bool backlight)
{
    ...
    tmp = backlight ? GPIO_PIN_SET : GPIO_PIN_RESET;

	HAL_GPIO_WritePin(DISPL_LED_GPIO_Port, DISPL_LED_Pin, tmp);
#endif
}
...
static void ili9341_send_cmd(uint8_t cmd)
{
    HAL_GPIO_WritePin(DISPL_DC_GPIO_Port, DISPL_DC_Pin, GPIO_PIN_RESET);  /*Command mode*/
    //disp_spi_send_data(&cmd, 1);
    HAL_SPI_Transmit(&hspi1, &cmd, 1, DISPL_SPI_TO);
}

static void ili9341_send_data(void * data, uint16_t length)
{
    HAL_GPIO_WritePin(DISPL_DC_GPIO_Port, DISPL_DC_Pin, GPIO_PIN_SET); /*Data mode*/
    //disp_spi_send_data(data, length);
    HAL_SPI_Transmit(&hspi1, data, length, DISPL_SPI_TO);
}
